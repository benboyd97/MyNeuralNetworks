...
class Regressor():
    ...
    def predict(self, x, pre_proc=True):
        ...
        return self.model(tensor_x).cpu().detach().numpy()
...
def RegressorHyperParameterSearch(x_train,y_train,x_val,y_val,x_test,y_test):
    ...
    c=0
    total_comb=len(hidden_layers_array)*len(neurons_array)*len(lr_array)*len(n_array)
        ...
        for j in range(len(neurons_array)):
            for k in range(len(lr_array)):
                for l in range(len(n_array)):
                    ...
                    regressor.fit(x_train, y_train,x_val=x_val,y_val=y_val,mini_batch_size=bs,early_stop_n=n_array[l])
                    ...
                    c+=1
                    print('Trained ',c, ' out of ', total_comb)
...
def example_main():
    ...
    regressor.fit(x_train, y_train,x_val=x_val,y_val=y_val,mini_batch_size=bs,early_stop_n=5)
    ...
    best_regressor.fit(x_train, y_train,x_val=x_val,y_val=y_val,mini_batch_size=bs,early_stop_n=best_early_stop_n)
